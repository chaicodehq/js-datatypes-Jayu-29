...
export function formatChaiMenu(items) {
  ...
  const validItems = items.filter(item => item.name !== "" && item.price > 0 && typeof item.name === "string");
  return validItems.map(items => `${items.name.toUpperCase()} - Rs.${items.price}`).join(" | ");
}
